...
if ($ExpectUnavailable) {
    try {
        Invoke-RestMethod "$JavaBaseUrl/api/v1/germination-drafts" -Method Post -ContentType 'application/json' -Body '{"question":"demo"}' -TimeoutSec 10 | Out-Null
...
foreach ($withBatch in @($false, $true)) {
    ...
    $result = Invoke-RestMethod "$JavaBaseUrl/api/v1/germination-drafts" -Method Post -ContentType 'application/json; charset=utf-8' -Body ([Text.Encoding]::UTF8.GetBytes($body)) -TimeoutSec 10
...
try {
    Invoke-RestMethod "$JavaBaseUrl/api/v1/germination-drafts" -Method Post -ContentType 'application/json' -Body '{"question":" "}' -TimeoutSec 10 | Out-Null
