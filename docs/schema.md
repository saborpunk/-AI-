...
| role | CUSTOMER默认 / MERCHANT；内部创建固定客户，角色授权后续实现 |
| status | ENABLED默认 / DISABLED；本阶段不提供状态管理API |
...
user_id外键RESTRICT，不级联删除会话。内部归属方法成功后增加version和updated_at，保留原title/notes/created_at；已归属或版本过期409。V2没有修改已有数据归属，V3再按真实身份核实迁移。
