...
flowchart LR
    ...
    W --> S["Service：业务校验与事务"]
    S --> M["MyBatis Mapper：参数绑定SQL"]
    M --> D[("MySQL：分类、文章、会话")]
```

三个新模块独立走三层架构，没有AI依赖；实体使用普通Java类，输入使用独立DTO。无无意义Service接口、通用CRUD基类或额外组件。当前直接返回不含敏感字段的实体；用户模块加入时必须用专门响应DTO排除密码。
...
分类状态当前是管理标记，不联动文章发布；会话CLOSED仍可修改元数据或重新设OPEN。V1尚无用户归属，只有合成演示数据；V2/V3引入用户与归属迁移后，才能进行多用户试用。

## 持久化与接口边界

新增表及外键见[schema](schema.md)。新接口见[api](api.md)。旧表和接口字段保持原约定，不迁移或删除旧数据。
...
| 版本 | 内容 | 验收门槛 |
| --- | --- | --- |
| V1（本次完成） | 三个传统CRUD，MyBatis + 真实MySQL | 正常/异常CRUD、条件分页、关联保护、重启读回；无需AI服务 |
| V2（待确认） | 持久层整理/MyBatis-Plus、用户表及归属迁移 | Entity→Mapper→SQL→Table映射、旧数据兼容；密码仅哈希保存 |
